...
archivo_corpus_entrenamiento_sta = path_proyecto + '/corpus/Ventana/Entrenamiento/supertag_all_training.csv'
archivo_corpus_pruebas_sta = path_proyecto + '/corpus/Ventana/Pruebas/supertag_all_pruebas.csv'
...
archivo_acc_sta = './accuracy_st_all.png'
archivo_loss_sta = './loss_st_all.png'

log = 'Log de ejecucion:\n-----------------\n'
log += '\nTareas: NER, Chunking, POS Simple y Super Tagging (reducido y completo)'
...
third_layer_sta = Dense(units=unidades_ocultas_capa_3_sta,
                    use_bias=True,
                    kernel_initializer=TruncatedNormal(mean=0.0, stddev=0.1, seed=3),
                    bias_initializer=Constant(value=0.1))
...
# ST All
model_sta = Sequential()

model_sta.add(embedding_layer)
model_sta.add(Flatten())
model_sta.add(second_layer)
model_sta.add(Activation("relu"))
model_sta.add(third_layer_sta)
model_sta.add(Activation("softmax"))
...
model_str.summary()
model_sta.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
model_sta.summary()
...
# Abro el archivo con casos de entrenamiento
df = pd.read_csv(archivo_corpus_entrenamiento_sta, sep=',', skipinitialspace=True, header=None, quoting=3)
largo = len(df)

# Separo features de resultados esperados
x_train_sta = np.array(df.iloc[:largo,:11])
y_train_sta = np.array(df.iloc[:largo,11:])
...
# Abro el archivo con casos de prueba
df = pd.read_csv(archivo_corpus_pruebas_sta, sep=',', skipinitialspace=True, header=None, quoting=3)
largo = len(df)

# Separo features de resultados esperados
x_test_sta = np.array(df.iloc[:largo,:11])
y_test_sta = np.array(df.iloc[:largo,11:])
...
history_sta = {'acc':[0]*cant_iteraciones, 'val_acc':[0]*cant_iteraciones, 'loss':[0]*cant_iteraciones, 'val_loss':[0]*cant_iteraciones}
...
for i in range(cant_iteraciones):
	...
	print 'ST All'
	history_aux = model_sta.fit(x_train_sta, y_train_sta, validation_data=(x_test_sta, y_test_sta), epochs=1, batch_size=100, verbose=2)
	history_sta['acc'][i] = history_aux.history['acc'][0]
	history_sta['val_acc'][i] = history_aux.history['val_acc'][0]
	history_sta['loss'][i] = history_aux.history['loss'][0]
	history_sta['val_loss'][i] = history_aux.history['val_loss'][0]
...
log += '\n\nSuperTagging Completo\n--------'
log += '\n\nAccuracy entrenamiento inicial: ' + str(history_sta['acc'][0])
log += '\nAccuracy entrenamiento final: ' + str(history_sta['acc'][-1])
log += '\n\nAccuracy validacion inicial: ' + str(history_sta['val_acc'][0])
log += '\nAccuracy validacion final: ' + str(history_sta['val_acc'][-1])

log += '\n\nLoss entrenamiento inicial: ' + str(history_sta['loss'][0])
log += '\nLoss entrenamiento final: ' + str(history_sta['loss'][-1])
log += '\n\nLoss validacion inicial: ' + str(history_sta['val_loss'][0])
log += '\nLoss validacion final: ' + str(history_sta['val_loss'][-1])
...
# summarize history for accuracy
plt.plot(history_sta['acc'])
plt.plot(history_sta['val_acc'])
plt.title('model accuracy')
plt.ylabel('accuracy')
plt.xlabel('epoch')
plt.legend(['train', 'test'], loc='upper left')
#plt.show()
plt.savefig(archivo_acc_sta, bbox_inches='tight')

# summarize history for loss
plt.clf()
plt.plot(history_sta['loss'])
plt.plot(history_sta['val_loss'])
plt.title('model loss')
plt.ylabel('loss')
plt.xlabel('epoch')
plt.legend(['train', 'test'], loc='upper left')
#plt.show()
plt.savefig(archivo_loss_sta, bbox_inches='tight')
plt.clf()
