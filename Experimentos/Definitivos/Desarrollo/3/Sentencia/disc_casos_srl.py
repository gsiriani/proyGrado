...
def main(tarea, precalculado = False):
	...
	x_test_a, x_test_b, x_test_c, y_test, x_d = cargarCasos(archivo_corpus_pruebas)
	...
	for f in range(unidades_ocultas_capa_3 - 1):
		salida[f] = {}
		for c in range(unidades_ocultas_capa_3 - 1):
			if f != c:
				salida[f][c] = []
	...
	for p in range(len(y_pred)):
		c = y_pred[p]
		f = y_true[p]
		if f < unidades_ocultas_capa_3 -1 and c < unidades_ocultas_capa_3-1 and f != c:
			salida[f][c].append(str(x_d[p]) + ' ' + str(x_test_a[p]))



	print 'Escribo archivos'
	...
	for f in range(unidades_ocultas_capa_3 - 1):
		for c in range(unidades_ocultas_capa_3 - 1):
			if f != c:
				...
				for l in salida[f][c]:
					texto += str(l) + '\n'
				open("casos/casos_" + str(f) + "-" + str(c) + ".txt", "w").write(texto)
