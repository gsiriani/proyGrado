...
archivo_corpus_entrenamiento = path_proyecto + '/corpus/Ventana/Entrenamiento/ner_training_reducido.csv'
...
oraciones = []
oracion_actual = {'x':[], 'y':[], 'largo':0, 'correctos':0}
...
print 'Calculando resultados por oraciones...'

resultados = []
largo = len(oraciones)
contador = 0
totalmente_correctas = 0
for o in oraciones:
	print_progress(contador, largo, prefix = 'Progreso:', suffix = 'Completado', bar_length = 50)
	contador += 1
	predicciones = model.predict(o['x'], batch_size=100, verbose=0)
	for p in range(o['largo']):
		esperado = o['y'].tolist().index(max(o['y'].tolist()))
		y_pred = predicciones[p].tolist().index(max(predicciones[p].tolist()))
		if esperado == y_pred:
			o['correctos'] += 1
	if o['largo'] == o['correctos']:
		totalmente_correctas += 1
...
log += '\n\nPorcentaje de oraciones completamente correctas: ' + str((totalmente_correctas*100)/largo)
