...
def main(tarea, cantidad_iteraciones = 20, precalculado = False):
	...
	train = model.evaluate({'main_input': x_train_a, 'aux_input': x_train_b}, {'softmax_layer': y_train}, batch_size=200, verbose=0)
	acc_ini = train[1]
	loss_ini = train[0]
	test = model.evaluate({'main_input': x_test_a, 'aux_input': x_test_b}, {'softmax_layer': y_test}, batch_size=200, verbose=0)
	val_acc_ini = test[1]
	val_loss_ini = test[0]	
	...
	history = model.fit({'main_input': x_train_a, 'aux_input': x_train_b}, {'softmax_layer': y_train}, 
