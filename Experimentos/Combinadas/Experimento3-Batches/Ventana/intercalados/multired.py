...
print 'Entrenando...'
batch_ner = int(len(x_train_ner)/divisor)
batch_chunking = int(len(x_train_chunking)/divisor)
batch_pos = int(len(x_train_pos)/divisor)
batch_st = int(len(x_train_str)/divisor)
...
for i in range(cant_iteraciones):		
	...
	for j in range(divisor):
		...
		print 'NER'
		history_aux = model_ner.fit(x_train_ner[batch_ner*j: batch_ner*(j+1)], y_train_ner[batch_ner*j: batch_ner*(j+1)], epochs=1, batch_size=100, verbose=0)

		print 'CHUNKING'
		history_aux = model_chunking.fit(x_train_chunking[batch_chunking*j: batch_chunking*(j+1)], y_train_chunking[batch_chunking*j: batch_chunking*(j+1)], epochs=1, batch_size=100, verbose=0)

		print 'POS'
		history_aux = model_pos.fit(x_train_pos[batch_pos*j: batch_pos*(j+1)], y_train_pos[batch_pos*j: batch_pos*(j+1)], epochs=1, batch_size=100, verbose=0)

		print 'ST Reducido'
		history_aux = model_str.fit(x_train_str[batch_st*j: batch_st*(j+1)], y_train_str[batch_st*j: batch_st*(j+1)], epochs=1, batch_size=100, verbose=0)


	print 'EVALUANDO....'
	def evaluar(modelo, x_train, y_train, x_test, y_test):
		train = modelo.evaluate(x=x_train, y=y_train, batch_size=200, verbose=0)
		train_acc = train[1]
		train_loss = train[0]
		test = modelo.evaluate(x=x_test, y=y_test, batch_size=200, verbose=0)	
		test_acc = test[1]
		test_loss = test[0]
		return train_acc, train_loss, test_acc, test_loss

	history_ner['acc'][i], history_ner['loss'][i], history_ner['val_acc'][i], history_ner['val_loss'][i] = evaluar(model_ner, x_train_ner, y_train_ner, x_test_ner, y_test_ner)
	history_chunking['acc'][i], history_chunking['loss'][i], history_chunking['val_acc'][i], history_chunking['val_loss'][i] = evaluar(model_chunking, x_train_chunking, y_train_chunking, x_test_chunking, y_test_chunking)
	history_pos['acc'][i], history_pos['loss'][i], history_pos['val_acc'][i], history_pos['val_loss'][i] = evaluar(model_pos, x_train_pos, y_train_pos, x_test_pos, y_test_pos)
	history_str['acc'][i], history_str['loss'][i], history_str['val_acc'][i], history_str['val_loss'][i] = evaluar(model_str, x_train_str, y_train_str, x_test_str, y_test_str)
