...
x_train_b = [ np.array([np.array([i-l[0]]) for i in range(len(l)-1)]) for l in x_train] # Matriz que almacenara distancias a la palabra a analizar
x_train_a = np.array(x_train_a[:51])
x_train_b = np.array(x_train_b[:51])
y_train = np.array(y_train[:51])

for l in range(len(x_train_a)):
    if len(x_train_a[l]) <> len(x_train_b[l]):
        print 'Discrepancia en linea: ' + str(l)
...
history = model.fit({'main_input': x_train_a, 'aux_input': x_train_b}, {'softmax_layer': y_train}, epochs=10, batch_size=25, verbose=2)
